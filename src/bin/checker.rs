...
use ProductManager::{opendb, sort, sortpromo, compare, dbpath, load};
...
pub fn main() -> Result<()> {
    let path = dbpath()?;
    let conn = opendb(&path).context("Failed to open database")?;
    ...
    let conn = opendb(&path).context("Failed to opendatabase")?;
